   ...
   return new Promise((resolve, reject) => {
      if (dataObject.method === 'post') {
         ...
         if (receiverId && senderID) {
            const query = "SELECT m.*,s.* FROM message_data m,user_data s WHERE" +
              " ((sender_email LIKE '" + senderID + "' AND receiver_email LIKE '" + receiverId + "')" +
              " OR (receiver_email LIKE '" + senderID + "' AND sender_email LIKE '" + receiverId + "')) AND m.sender_email LIKE s.email";
...
messagesHandler.recent = function (dataObject) {
   return new Promise((resolve, reject) => {
      if (dataObject.method === 'put') {
         ...
         const query = "SELECT m.*,s.* FROM message_data m, user_data s " +
           "WHERE id IN (SELECT max(id) as id FROM" +
           " (SELECT id,receiver_email as email FROM " +
           "message_data WHERE sender_email = '" + email + "' " +
           "UNION SELECT id, sender_email as email FROM message_data " +
           "WHERE receiver_email LIKE '" + email + "') as T GROUP BY " +
           "email) and s.email like sender_email";
         database.query(query).then(msgData => {
            resolve([200, {'res': msgData}]);
         }).catch(err => {
